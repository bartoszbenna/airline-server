...
export class ReservationService {
      ...
      if (this.verifyResCreationData(data)) {
        try {
          ...
          const reservation: Reservation = {
            reservationNumber: await this.createReservationNumber(),
          ...
          for (let flight of basket.flights) {
            ...
            for (let passenger of data.passengers) {
              const seatObject = passenger.seats.find(
                (element) => element.flightId == flight._id,
              );
              let seat: string = '';
              if (seatObject != undefined && seatObject.seat != '') {
                if (
                  !(await this.searchService.isSeatOccupied(
                    seatObject.flightId,
                    seatObject.seat,
                  ))
                ) {
          ...
          for (let flight of reservation.flights) {
            ...
            for (let passenger of flight.passengers) {
              ...
              reservation.totalPrice +=
                passenger.seat != '' ? this.seatPrice : 0;
            }
  ...
  public async getReservations(token: string) {
    const reservationPromise = new Promise<ReservationDocument[]>(
  ...
  private async createReservationNumber(): Promise<string> {
    const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    let number = '';
    for (let i = 0; i < 6; i++) {
      number += characters[Math.floor(Math.random() * 35)];
    }
    const existing = await this.reservationModel.find({
  ...
  public async confirmReservation(reservationNumber: string) {
    const reservationConfirmationPromise = new Promise(
      async (resolve, reject) => {
        try {
  ...
  public async removeUnconfirmedReservations() {
    try {
      ...
      const unconfirmedReservations = await this.reservationModel.find({
        reservationDate: { $lte: expiryDate },
        isConfirmed: false,
      });
