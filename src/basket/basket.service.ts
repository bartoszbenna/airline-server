...
export class BasketService {
    ...
    public async getBasket(token: string) {
        const basketPromise = new Promise<BasketDocument>(async (resolve, reject) => {
            ...
            let matchingUser: UserDocument;
            try {
                matchingUser = await this.loginService.getUserInfo(tokenPayload.userId);
            }
            catch(error) {
                reject('databaseError')
                return;
            }
            if (matchingUser != null && matchingUser != undefined && tokenPayload.currentKey == matchingUser.currentKey) {
                let existingBasket: BasketDocument;
                try {
                    existingBasket = await this.basketModel.findOne({userId: matchingUser._id})
                }
                catch (error) {
                    reject('databaseError');
                    return;
                ...
                if (existingBasket != null && existingBasket != undefined) {
                    if (existingBasket.expiryTime > new Date()) {
                        ...
                        for (let flight of existingBasket.flights) {
                            await this.searchService.changeAvailability(flight._id, flight.adult + flight.child);
                        }
    ...
    public async createBasket(userId: string) {
        const basketCreationPromise = new Promise<BasketDocument>(async (resolve, reject) => {
            const expiryTime = new Date();
            expiryTime.setMinutes(expiryTime.getMinutes() + this.basketExpiryTimeInMinutes);
            try {
                const basket = await this.basketModel.create({userId: userId, flights: [], expiryTime: expiryTime, totalPrice: 0});
                resolve(basket)
            }
            catch (error) {
                reject('databaseError');
    ...
    public async uploadBasket(flights: IFlight[], token: string) {
        ...
        const basketAdditionPromise = new Promise<BasketDocument>(async (resolve, reject) => {
            ...
            try {
                for (let flight of flights) {
                    ...
                    try {
                        const verifiedFlight = await this.searchService.getFlightById(flight._id);
                        if (verifiedFlight.available < flight.adult + flight.child) {
                            reject('flightNotAvailable');
                        }
                        else {
                            await this.searchService.changeAvailability(verifiedFlight._id, (flight.adult + flight.child) * -1)
                            basket.flights.push({
                            ...
                            basket.totalPrice += (flight.adult + flight.child + flight.infant) * verifiedFlight.price;
                        }
                    }
                    catch (error) {
    ...
    public async removeExpiredBaskets() {
            try {
                const expiredBaskets = await this.basketModel.find({expiryTime: {$lte: new Date()}});
